...
vis_mat$MEN1 = vis_mat$MEN1 + -1*min(vis_mat$MEN1) + 1
...
p = p + annotate("text", x=1:ncol(expr_raw_selection),y = 9,parse=TRUE, label = label_vec, color = col_vec, size = 4.5 )
p = p + annotate("text", x=10,y = 8, label= "Wilcoxon-Smith test p-value = 4E-2", size = 4.5 )
...
#svg("~/Downloads/MAPTor-NET_plots_20_10_2022/SM_Figure_5_Waterfall_validation.svg", width = 8, height = 5)
p + ylab("Log TPM")
dev.off()
